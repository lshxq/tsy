<template lang="pug">
  .home-mai
    p
      .h1 TSY VUE
      p tsy vue是一套为了快速迭代项目，而整理一个基于element-ui的扩充组件库，element-ui本身是和业务无关的基础组件库，在高质量显示的同时在一定程度上提高了开发效率，不过还不够，通过引入tsyvue，能极大的提高开发效率，进一步提升项目的标准化、以及一致性。
      p 通过npm 进行安装
      .code-block
      ...
      p 分页查询（pagin） 和 表单（form） 系列组件 会直接通过 axios和后端做通讯，拉取、提交数据。
    ...
    p.mt100
      ...
      p.red 为了提升模板的开发效率，html模板我推荐使用pug语言
      p pug 就是之前的 jade，早在jsp的时代，jade就是html模板的首选，主要 思想是 通过 css选择器 配合 缩进 格式 来书写极简的html， 通过优雅的格式，取消了繁琐的开始和闭合标签。习惯后端yml作为配置文件的开发者，会很快接受pug。
      ...
      p sy-pagin-data 就是这样一个组件，该组件在使用时大概是这个样子，可以看到代码相当简洁，组件用户只需要关注每一条数据的渲染即可。检索条件变化以及分页都会自动拉取对应数据。
